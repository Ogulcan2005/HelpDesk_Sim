"""
Formulieren voor accountbeheer in de admin en op docentpagina's.

Overzicht:
    UserAccountFormMixin  → gedeelde basis (e-mail, wachtwoord, rol)
    StudentAdminForm      → leerling beheren in Django admin
    TeacherAdminForm      → docent beheren in Django admin
    StudentInlineForm     → leerling toevoegen binnen een klas (admin inline)
    TeacherInlineForm     → docent toevoegen binnen een klas (admin inline)
    TeacherAddStudentForm → student toevoegen op klasdetailpagina
    TeacherCreateStudentForm → student aanmaken op docentenbeheerpagina
    TeacherEditStudentForm   → student bewerken op docentenbeheerpagina
"""
...
class UserAccountFormMixin(forms.ModelForm):
    """
    Gedeelde basis voor admin-formulieren.

    Voegt e-mail, wachtwoord en (optioneel) rol toe bovenop het
    Student/Teacher-model. Zorgt dat User + profiel samen worden opgeslagen.
    """
    show_role = True
    ...
    def clean_email(self):
        """Voorkomt dubbele e-mailadressen (username = e-mail in dit project)."""
    ...
    def clean(self):
        """Controleert docent-per-klas regel en verplicht wachtwoord bij nieuw account."""
        cleaned = super().clean()
    ...
    def _ensure_user(self, instance, email, password):
        """Maakt een User aan of werkt bestaande bij (login-account)."""
        if instance.user_id:
            user = instance.user
            user.username = email
            user.email = email
    ...
    def save(self, commit=True):
        """Slaat User + Student/Teacher op, inclusief eventuele rolwissel."""
...
class StudentAdminForm(UserAccountFormMixin):
    """Formulier in Django admin om leerlingen aan te maken/bewerken (met rolkeuze)."""
    default_role = ROLE_STUDENT
...
class TeacherAdminForm(UserAccountFormMixin):
    """Formulier in Django admin om docenten aan te maken/bewerken (met rolkeuze)."""
...
class StudentInlineForm(UserAccountFormMixin):
    """Compact formulier: leerling toevoegen als rij op de klas-pagina in admin."""
    show_role = False
...
class TeacherInlineForm(UserAccountFormMixin):
    """Compact formulier: docent toevoegen als rij op de klas-pagina in admin."""
    show_role = False
...
class TeacherAddStudentForm(forms.Form):
    """
    Formulier op de klasdetailpagina om één student toe te voegen.

    Maakt User + Student aan en koppelt direct aan de opgegeven klas.
    """
...
class TeacherCreateStudentForm(TeacherAddStudentForm):
    """
    Formulier op /docent/studenten/ om een nieuwe student aan te maken.

    Uitbreiding van TeacherAddStudentForm met een klas-dropdown.
    """
...
class TeacherEditStudentForm(forms.ModelForm):
    """
    Formulier om een bestaande student te bewerken (/docent/studenten/<id>/bewerken/).

    Werkt zowel het Student-profiel als het gekoppelde User-account bij.
    """
    email = forms.EmailField(label='E-mail')
    password = forms.CharField(
        label='Wachtwoord',
