"""
Django admin-configuratie voor klassen, leerlingen en docenten.

Wat je hier kunt doen:
    - ClassGroup: klassen beheren, met studenten/docenten als inline-tabellen
    - Student:    alle leerlingaccounts bekijken/bewerken
    - Teacher:    alle docentaccounts bekijken/bewerken
    - Group:      rechten-groepen met overzicht van leden

Bulkactie 'Uit klas verwijderen': koppelt geselecteerde profielen los
van hun klas zonder het account te verwijderen.
"""
...
def remove_from_class(modeladmin, request, queryset):
    """Bulkactie: zet class_group op None (account blijft bestaan)."""
    queryset.update(class_group=None)


class StudentInline(admin.TabularInline):
    """Studenten als tabel-rijen op de ClassGroup-pagina in admin."""
    model = Student
...
class TeacherInline(admin.TabularInline):
    """Docenten als tabel-rijen op de ClassGroup-pagina in admin."""
    model = Teacher
...
class ClassGroupAdmin(admin.ModelAdmin):
    ...
    Toont StudentInline en TeacherInline zodat je leerlingen/docenten
    direct op de klas-pagina kunt toevoegen. Verwijderen in de inline
    koppelt los (class_group=None), verwijdert het account niet.
    """
...
class RoleSwitchAdminMixin:
    """
    Stuurt na een rolwissel (leerling ↔ docent) door naar het juiste
    admin-scherm, met een bevestigingsmelding.
    """
...
class StudentAdmin(RoleSwitchAdminMixin, admin.ModelAdmin):
    """Admin-lijst en bewerkpagina voor alle leerlingen."""
...
class TeacherAdmin(RoleSwitchAdminMixin, admin.ModelAdmin):
    """Admin-lijst en bewerkpagina voor alle docenten."""
...
class GroupAdmin(admin.ModelAdmin):
    """Aangepaste admin voor Django-rechtengroepen met ledenoverzicht."""
    def users(self, obj):
